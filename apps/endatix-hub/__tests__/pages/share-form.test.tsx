...
vi.mock('@/features/public-form/use-cases/get-active-definition.use-case', () => ({
  getActiveDefinitionUseCase: vi.fn()
}));
vi.mock('@/features/public-form/use-cases/get-partial-submission.use-case', () => ({
...
describe('ShareForm Page', async () => {
  ...
  it('displays form not found message when definition is not found', async () => {
    vi.mocked(getActiveDefinitionUseCase).mockResolvedValue(Result.error('Form not found'));
    vi.mocked(getPartialSubmissionUseCase).mockResolvedValue(Result.error('Submission not found'));
