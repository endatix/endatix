...
describe('uploadUserFilesUseCase', () => {
  ...
  it('should successfully upload files and return urls', async () => {
    ...
    vi.spyOn(StorageService.prototype, 'uploadToStorage').mockResolvedValue(mockUrl);
    vi.spyOn(StorageService.prototype, 'optimizeImageSize').mockResolvedValue(Buffer.from('optimized'));
