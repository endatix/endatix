...
const SubmissionsLabel: React.FC<SubmissionsLabelProps> = ({
  ...
  return (
    <div>
      <span className="text-2xl font-medium text-muted-foreground">{getFormattedSubmissionsCount()}</span>
      <span className="pl-2 text-sm text-muted-foreground">{submissionWord}</span>
    </div>
  )
}
      ...
      <div className="cursor-pointer">
        ...
        <CardContent className="grid gap-4 p-4">
          <div className="flex justify-between items-center">
            ...
            <Badge className="text-xs font-normal pointer-events-none" variant={form.isEnabled ? "default" : "secondary"}>
              {getFormLabel()}
            </Badge>
          </div>
        </CardContent>
      ...
      >
        <div className="flex justify-between w-full">
          <div className="flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
            <Link href={`forms/${form.id}`} className="text-sm text-muted-foreground hover:text-foreground inline-flex items-center cursor-pointer">
              <Pencil className="w-4 h-4 mr-1" />
              Design
            </Link>
            <Link href={`/share/${form.id}`} target="_blank" className="text-sm text-muted-foreground hover:text-foreground inline-flex items-center cursor-pointer">
              <Link2 className="w-4 h-4 mr-1" />
              Share
            </Link>
            <Link 
              href={form?.submissionsCount ? `forms/${form.id}/submissions` : "#"} 
              className={cn(
                "text-sm text-muted-foreground inline-flex items-center",
                form?.submissionsCount ? "hover:text-foreground cursor-pointer" : "opacity-50 pointer-events-none cursor-default"
              )}
            >
