...
description: Safely migrate Endatix from JSON text columns to PostgreSQL native JSONB - step-by-step guidance for a zero-downtime production database migration.
sidebar_position: 7
---
...
- [PostgreSQL `pg_trgm` requirement](/docs/getting-started/system-requirements#postgresql-pg_trgm-extension) — separate from JSONB; required for data-list label search
