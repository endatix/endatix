...
3. Ensure your Identity Provider issues those claims in the access token used by the public form.
...
* `profile` is filtered through `ProfileSnapshotFields` before it becomes `SubmitterProfileSnapshot`.
