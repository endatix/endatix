...
The single-submission gate (`RestrictionKey` + `UX_Submissions_RestrictionKey`) also changed format when submitter identity was introduced:

| Era | `RestrictionKey` shape |
| --- | --- |
| Legacy | `SingleSubmission:Form:{formId}:User:{submittedBy}` |
| Current | `SingleSubmission:Form:{formId}:Submitter:{submitterId}` |

The unique index deduplicates **identical key strings only**. If legacy rows keep `:User:…` while new code writes `:Submitter:…`, the same respondent can create a second submission on a `LimitOnePerUser` form even though they represent one person. The handler fast-path (`SubmissionByFormIdAndSubmitterIdSpec`) helps only after `SubmitterId` is populated; rows with `SubmitterId IS NULL` still bypass that check.

**Recommended rollout strategy**

1. **Schema update:** Apply the additive migration (`Submitters` table + nullable `Submissions.SubmitterId` and related columns). This does **not** rewrite existing `RestrictionKey` values.
2. **Ensure `Submitters` exist:** Sections A–C below link submissions to **existing** submitter rows. They do not create submitters. Before backfill, ensure legacy identities have matching `Submitters` rows (for example via first authenticated submission after upgrade, a one-time import, or operator-driven provisioning). Unmatched legacy `SubmittedBy` values remain orphaned until a submitter row exists.
3. **Backfill `SubmitterId`:** Run sections A–D below.
4. **Backfill `RestrictionKey`:** Run section E to normalize gated rows to the `:Submitter:` format **before** or immediately after deploying code that writes the new shape—especially on forms with `LimitOnePerUser = true`.
5. **Normalize mirror:** Section D keeps `SubmittedBy` aligned with `SubmitterId`.
6. **Deprecate:** Once reads are fully migrated to `SubmitterId`, verify no orphaned records remain, then drop `SubmittedBy` in a future release.

Deploying application code that writes `:Submitter:…` keys **before** steps 3–4 completes leaves a window where legacy rows (`:User:…`, `SubmitterId IS NULL`) do not collide with new rows at the index and may not match the fast-path spec. Complete backfill (or accept that window on non-limited forms only) before relying on single-submission enforcement for returning respondents.
...
-- E) Normalize RestrictionKey to the Submitter-based format (run after SubmitterId backfill)
UPDATE "Submissions"
SET "RestrictionKey" = 'SingleSubmission:Form:' || "FormId" || ':Submitter:' || "SubmitterId"
WHERE "SubmitterId" IS NOT NULL
  AND "IsTestSubmission" = false
  AND "RestrictionKey" IS NOT NULL
  AND "RestrictionKey" NOT LIKE '%:Submitter:%';

```

### SQL Server Backfill Script

Use the same section order (A–E). Adjust types and concatenation for T-SQL:

```sql
-- A) SubmittedBy already matches a numeric Submitter.Id
UPDATE s
SET s.SubmitterId = CAST(s.SubmittedBy AS bigint)
FROM Submissions s
WHERE s.SubmitterId IS NULL
  AND s.SubmittedBy NOT LIKE '%[^0-9]%'
  AND EXISTS (
    SELECT 1
    FROM Submitters sub
    WHERE sub.Id = CAST(s.SubmittedBy AS bigint)
...
-- B) Legacy Endatix rows: SubmittedBy matches AppUser.Id
UPDATE s
SET s.SubmitterId = sub.Id
FROM Submissions s
INNER JOIN Submitters sub
  ON sub.AuthProvider = 'Endatix'
 AND sub.AppUserId = CAST(s.SubmittedBy AS bigint)
WHERE s.SubmitterId IS NULL
  AND s.SubmittedBy NOT LIKE '%[^0-9]%';
...
FROM Submissions s
INNER JOIN Submitters sub
  ON sub.AuthProvider = 'Keycloak'
 AND sub.ExternalSubjectId = s.SubmittedBy
WHERE s.SubmitterId IS NULL
  AND s.SubmittedBy IS NOT NULL
  AND s.SubmittedBy LIKE '%[^0-9]%';

-- D) Normalize the temporary mirror after backfill
UPDATE Submissions
SET SubmittedBy = CAST(SubmitterId AS nvarchar(64))
WHERE SubmitterId IS NOT NULL
  AND (SubmittedBy IS NULL OR SubmittedBy <> CAST(SubmitterId AS nvarchar(64)));

-- E) Normalize RestrictionKey to the Submitter-based format (run after SubmitterId backfill)
UPDATE Submissions
SET RestrictionKey = CONCAT('SingleSubmission:Form:', FormId, ':Submitter:', SubmitterId)
WHERE SubmitterId IS NOT NULL
  AND IsTestSubmission = 0
  AND RestrictionKey IS NOT NULL
  AND RestrictionKey NOT LIKE '%:Submitter:%';
```
...
Run these queries before planning the final drop of the `SubmittedBy` column. Each should return `0`.

**Orphaned legacy identifiers (no resolved submitter):**
...
**Legacy RestrictionKey shape still present after backfill:**
...
On SQL Server, replace quoted identifiers and use `IsTestSubmission = 0` in the second query.
