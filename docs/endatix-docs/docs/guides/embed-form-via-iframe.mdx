...
injecting the script, with **Open HTTP playground** linking this process's own HTTP port
(`:5000` on the default launch profile). Hub serves `embed.js` and the iframe; the API origin is the
...
(`:3000`). Open the playground on the API's HTTP address instead — **Open HTTP playground** on the
card links it, or use `http://localhost:5000/dev/embed-host` on the default launch profile. Confirm
`data-form-id` is a published form.
