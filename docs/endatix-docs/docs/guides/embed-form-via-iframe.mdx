...
injecting the script. **Open HTTP playground** uses this process's HTTP port from Kestrel
(default launch profile: `:5000`). Hub serves `embed.js` and the iframe; the API origin is the host — same
