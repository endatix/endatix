...
description: Learn how to prefill Endatix forms with existing data and share submission links using access tokens for controlled, read-only access.
sidebar_position: 6
---
...
Endatix provides two endpoints for creating submissions, each suited for different scenarios:

### Option 1: Public Submission Creation

Use the **public endpoint** when creating anonymous submissions or when you don't need to track who created the submission.
...
- Cannot specify `submittedBy` field
- Ideal for anonymous form submissions
- May require reCAPTCHA validation (if configured)

### Option 2: Authenticated Submission Creation (On Behalf)

Use the **authenticated endpoint** when you need to create submissions on behalf of specific users and track who they're for.

**Endpoint:** `POST /api/forms/{formId}/submissions/onbehalf`

**Authentication:** Required with `Submissions.CreateOnBehalf` permission

**Request Example:**
```http
POST /api/forms/1442869143157080064/submissions/onbehalf
Content-Type: application/json
...
**Key Features:**
- Requires authentication and `Submissions.CreateOnBehalf` permission
- Can specify `submittedBy` field to track the user the submission is for
- No reCAPTCHA required (authenticated requests)
- Ideal for admin/staff creating submissions on behalf of users

**Common Use Cases:**
- Healthcare staff pre-filling patient forms
- Customer service representatives creating submissions for customers
- Admin users bulk-importing historical data with proper attribution
- Support staff creating submissions during assisted onboarding

### Choosing the Right Endpoint

| Scenario | Recommended Endpoint |
|----------|---------------------|
| Anonymous user submitting a form | Public endpoint |
| Embedded forms on public websites | Public endpoint |
| Admin creating submission for a specific user | Authenticated (onbehalf) endpoint |
| Bulk import with user attribution | Authenticated (onbehalf) endpoint |
| Staff-assisted form completion | Authenticated (onbehalf) endpoint |
...
You can use either the **public endpoint** or the **authenticated (onbehalf) endpoint** to create the submission, depending on your scenario. See [Creating Submissions](#creating-submissions) for details on both options.

**Example using the public endpoint:**
...
> **Note**: If you're a staff member creating this submission on behalf of a specific user (e.g., a patient, customer, or client), use the authenticated `/onbehalf` endpoint instead and include the `submittedBy` field to track who the submission is for.
...
- `POST /api/forms/{formId}/submissions/onbehalf` - Create a submission on behalf of another user (authenticated)
