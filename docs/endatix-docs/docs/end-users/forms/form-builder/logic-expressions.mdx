...
### Type Conversion in Expressions: the `#` Prefix

Whenever you reference a question with `{question_name}` inside an expression, SurveyJS inspects the stored answer and automatically converts it to a number or boolean if it *looks* like one, before the rest of the expression ever sees it. This is what lets `{q1} + {q2}` add two answered numbers together instead of concatenating them as text (`"1" + "2"` becomes `3`, not `"12"`).

The conversion is silent and happens for every function and operator equally, including custom functions like `regexMatch`. It also has an inconsistency worth knowing about: a value with **leading zeros** (`"00351..."`) is protected and kept as a string, but a value with a **leading `+`** (`"+351912345678"`) is not — SurveyJS parses it as the number `351912345678`, and the `+` is gone by the time your expression runs. Trailing formatting, thousands separators, and similar text-shaped-like-a-number cases can be lost the same way.

To reference the raw, unconverted string value instead, prefix the question name with `#`:

```javascript
{#question_name}
```

| Reference | Value stored: `"+351912345678"` |
|-----------|----------------------------------|
| `{question_name}` | `351912345678` (number — the `+` is stripped) |
| `{#question_name}` | `"+351912345678"` (string — unchanged) |

Use `{#question_name}` any time an expression depends on the *exact text* of an answer rather than its numeric value — most commonly with `regexMatch` (see below), but the same rule applies to `contains`, string concatenation, or any custom function.
...
The `regexMatch` function tests whether a question value matches a regular expression pattern. Use it in `visibleIf`, `enableIf`, and other conditional expressions when you need regex-based show/hide or enable/disable logic beyond native question validators. To check an answer's format, use a [Regex validation rule](validation#regular-expressions) instead.

`regexMatch` is intended for **single-value text or numeric questions**. For checkbox, matrix, or other multi-value question types, match against individual answers (for example, a specific row or choice) rather than the whole question value.

#### Syntax
```javascript
regexMatch({#question_name}, '^pattern$')
regexMatch({#question_name}, '^pattern$', 'flags')
```

| Parameter | Description |
|-----------|-------------|
| `value` | The question value to test — always reference it as `{#question_name}` (see [Type Conversion in Expressions](#type-conversion-in-expressions-the--prefix) above) |
| `pattern` | Regular expression pattern as a string |
| `flags` | Optional regex flags (e.g. `'i'` for case-insensitive) |

:::warning[Always Use the Number Sign Prefix]
Reference the question value as `{#question_name}`, not `{question_name}`. Without the `#`, SurveyJS may silently convert an answer that looks numeric — including anything with a leading `+` — into a JS number before `regexMatch` ever runs, breaking any pattern anchored on that formatting (e.g. `^\+`). This applies even to questions of type `text`; the conversion is based on what the *value* looks like, not the question type.
:::

#### JSON Examples

Show a follow-up question only when the phone number matches a Korean mobile format:
```json
{
  "type": "text",
  "name": "followUp",
  "title": "Thank you for your valid number",
  "visibleIf": "regexMatch({#phone}, '^01[016789]-?\\d{3,4}-?\\d{4}$') == true"
}
```

Enable a submit button only when the user enters a valid code:
```json
{
  "type": "text",
  "name": "refCode",
  "title": "Reference Code",
  "enableIf": "regexMatch({#refCode}, '^[A-Z]{3}-\\d{4}$') == true"
}
```
...
```json
{
  "type": "text",
  "name": "confirm",
  "visibleIf": "regexMatch({#answer}, '^(yes|no)$', 'i') == true"
}
```

Match a Portuguese mobile number that may be entered with a leading `+`:
```json
{
  "type": "text",
  "name": "followUp",
  "visibleIf": "regexMatch({#phone}, '^((?:\\+|00)351)? ?(?:9[1236]\\d{7}|2\\d{8}|30\\d{7})$') == true"
}
```

:::info[Edge Cases and JSON Escaping]
- Returns `false` (never throws) when the value is empty, `null`, `undefined`, or an empty array (for example, a cleared checkbox).
- Returns `false` when the pattern is missing, not a string, or invalid regex syntax.
- Non-string scalar values (e.g. numbers) are coerced to strings before matching; `0` is treated as a valid answer.
- In JSON, backslashes must be escaped once: write `\\d` for a digit class in the pattern string.
:::
...
<table style={{ width: '100%', backgroundColor: 'var(--ifm-background-color)' }}>
  ...
  <tbody style={{ backgroundColor: 'transparent' }}>
    ...
    <tr style={{ backgroundColor: 'transparent' }}>
      <td><code>#</code></td>
      <td>Disables type conversion for a referenced value (e.g., string values "true", "false", "123", or "+123" won't be converted to corresponding Boolean and numeric values). See <a href="#type-conversion-in-expressions-the--prefix">Type Conversion in Expressions</a>.</td>
      <td><code>"&#123;#q1&#125;"</code></td>
    </tr>
  </tbody>
</table>
...
Endatix Hub is powered by the SurveyJS form engine. While all standard SurveyJS expressions are supported, Endatix Hub includes exclusive functions - such as `anyAnswered`, `anyAnsweredByPrefix`, and `regexMatch` - that may not be compatible with standalone SurveyJS implementations.
