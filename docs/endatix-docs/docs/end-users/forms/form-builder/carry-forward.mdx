...
:::warning[Lazy-loaded sources]

Lazy-loaded sources (for example, **Choices from data list**) always contribute **Selected** options only — including selections that are not on the currently loaded page — even when the mode is **All** or **Unselected**. See [Limitations](#limitations-and-mutual-exclusion).
...
<img src={require('./carry-forward-configuration.webp').default} className="legacy-screenshot" width="549" alt="Carry forward configuration screen" />
...
| Mode | Behavior |
| --- | --- |
| **All** | Copies every choice option defined on each source question, regardless of what the respondent selected. Requires a fully loaded (inline) choice catalog on that source. |
| **Selected** | Copies only options the respondent selected on each source. Updates dynamically as answers change. Works with both inline and lazy-loaded (data list) sources. |
| **Unselected** | Copies only options the respondent did **not** select on each source. Updates dynamically as answers change. Requires a fully loaded (inline) choice catalog on that source. |

:::info[Lazy-loaded / data-list sources]

Sources that load choices on demand (typically **Choices from data list**) never materialize the full catalog in the browser. For those sources, Carry Forward always copies **Selected** options only — including selections that are not on the currently loaded page — even when this setting is **All** or **Unselected**. Inline sources in the same destination still honor **All** / **Unselected**.
...
2. Collects choices according to the selected mode (**All**, **Selected**, or **Unselected**). Lazy-loaded sources always contribute **Selected** options only.
...
Carry Forward cannot be combined with these choice sources on the **same** (destination) question:
...
### **Lazy-loaded source questions**

A **source** question may use **Choices from data list** (or any lazy-loaded catalog). That is supported, with one mode constraint:

| Destination mode | Inline source | Lazy-loaded / data-list source |
| --- | --- | --- |
| **All** | Copies every defined choice | Contributes **Selected** only |
| **Selected** | Copies selected choices | Copies selected choices (including off-page) |
| **Unselected** | Copies unselected choices | Contributes **Selected** only |

This keeps large catalogs practical: the browser never needs to load thousands of unselected items to build the destination list.
...
<td><code>string</code></td>
<td><code>"all"</code></td>
<td><code>"all"</code>, <code>"selected"</code>, or <code>"unselected"</code>. Controls which options are copied from each source. Lazy-loaded sources always contribute as <code>"selected"</code>.</td>
</tr>
<tr style={{ backgroundColor: 'transparent' }}>
...
<td><code>"all"</code></td>
<td>All</td>
<td>Copy every choice from each source question. Lazy-loaded sources contribute selected choices only.</td>
</tr>
<tr style={{ backgroundColor: 'transparent' }}>
<td><code>"selected"</code></td>
<td>Selected</td>
<td>Copy only choices the respondent selected on each source. Supported for inline and lazy-loaded sources.</td>
</tr>
<tr style={{ backgroundColor: 'transparent' }}>
<td><code>"unselected"</code></td>
<td>Unselected</td>
<td>Copy only choices the respondent did not select on each source. Lazy-loaded sources contribute selected choices only.</td>
</tr>
</tbody>
</table>
