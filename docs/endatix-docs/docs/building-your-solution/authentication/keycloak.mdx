...
description: Complete guide to setting up Keycloak authentication for your Endatix form management system's Single-Sign-On needs
sidebar_position: 2
---
...
4. **Login settings**:
   ...
   - Valid post logout redirect URIs:
     - Development: `http://localhost:3000/signin`
     - Production: `https://yourdomain.com/signin`
...
1. Go to **"Clients"** → **"endatix-hub"** → **"Credentials"**
...
AUTH_KEYCLOAK_CLIENT_SECRET=your_client_secret_from_keycloak
AUTH_KEYCLOAK_ISSUER=http://localhost:8080/realms/endatix
AUTH_URL=http://localhost:3000
```

The rest of Hub env (API URL, `SESSION_SECRET`, `AUTH_SECRET`) is on [Hub environment variables](/docs/developers/hub/environment).
...
#### AUTH_KEYCLOAK_CLIENT_SECRET
  ...
  - Example: `AUTH_KEYCLOAK_CLIENT_SECRET=your_client_secret_from_keycloak`
...
#### AUTH_URL
  - The public base URL where Endatix Hub is served
  - Used by Auth.js callbacks and Keycloak federated sign-out
  - The Keycloak client must allow `${AUTH_URL}/signin` as a **Valid post logout redirect URI**
  - Example: `AUTH_URL=http://localhost:3000`

### 2.2 Configure Federated Sign-Out

Endatix Hub signs out locally first, then redirects Keycloak users to the Keycloak OIDC end-session endpoint. This clears the Keycloak SSO session so the next sign-in prompts for credentials instead of silently reusing the IdP session.

To enable this flow, configure the `endatix-hub` Keycloak client with a matching post-logout redirect URI:

```text
http://localhost:3000/signin
```

For production, use your Hub domain:

```text
https://yourdomain.com/signin
```

If Hub is hosted under a base path, include it in the URI, for example:

```text
https://yourdomain.com/app/signin
```

### 2.3 Register Provider in auth.ts
...
1. Sign in with Keycloak
2. Sign out from Endatix Hub
3. Verify the browser is redirected through Keycloak logout and returns to `/signin`
4. Click "Sign in with Keycloak" again and confirm Keycloak prompts for credentials instead of silently reusing the previous SSO session
...
- For logout errors, verify **Valid post logout redirect URIs** includes `${AUTH_URL}/signin`
