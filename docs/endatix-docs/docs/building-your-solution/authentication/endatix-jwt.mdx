...
Use `{JWT_SIGNING_KEY}` or user secrets — never commit production signing keys. See [auth settings](/docs/configuration/settings/auth-settings) for generating and storing a local key.
