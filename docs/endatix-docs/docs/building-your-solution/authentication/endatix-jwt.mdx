...
Use `{JWT_SIGNING_KEY}` or user secrets — never commit production signing keys. See [security configuration](/docs/configuration/security-configuration) for generating and storing a local key.
