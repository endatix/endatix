...
description: Deploy Endatix on your own infrastructure using Docker - covers container setup, environment configuration, port mapping, and service orchestration.
---
...
Full API, Hub, and database requirements (including PostgreSQL `pg_trgm`) are listed under [System Requirements](/docs/getting-started/system-requirements).
...
| `Endatix__Hub__HubBaseUrl` | Public Hub origin for email links — [API Hub URL](/docs/configuration/settings/hub-settings) | Optional |
...
The sample Compose stack uses **SQL Server**. If you point the API at your own **PostgreSQL** instance instead, enable the `pg_trgm` extension first — see [System Requirements](/docs/getting-started/system-requirements#postgresql-pg_trgm-extension).

### Endatix Hub (Next.js) Configuration
Configuration is passed via `.env` or container environment variables. See [Hub environment variables](/docs/developers/hub/environment) for runtime vs build, Helm, and the full catalog.
