...
If you need to serve the Hub or API under specific path prefixes (such as `/app` and `/api`), start with the [Subfolder Deployment](/docs/building-your-solution/deployment/subfolder-deployment.mdx) guide, then apply the reverse-proxy settings below.
...
`AUTH_TRUST_HOST=true` is strictly required when Auth.js runs behind a reverse proxy. See the [Endatix JWT authentication guide](/docs/building-your-solution/authentication/endatix-jwt.mdx) for more details on Hub authentication settings.
