...
If you need to serve the Hub or API under specific path prefixes (such as `/app` and `/api`), start with the [Subfolder Deployment](/docs/building-your-solution/deployment/subfolder-deployment) guide, then apply the reverse-proxy settings below. Hub needs `AUTH_URL` and usually `AUTH_TRUST_HOST=true` — [Hub environment variables](/docs/developers/hub/environment).
