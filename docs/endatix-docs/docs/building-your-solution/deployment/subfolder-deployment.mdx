...
For the required proxy headers and `Endatix:Hosting` API settings, you must read the [Reverse Proxy Deployment](/docs/building-your-solution/deployment/reverse-proxy-deployment) guide in conjunction with this page. Hub keys (`NEXT_PUBLIC_BASE_PATH`, `AUTH_URL`): [Hub environment variables](/docs/developers/hub/environment).
