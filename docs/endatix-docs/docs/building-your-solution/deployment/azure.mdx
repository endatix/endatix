...
Do you prefer a containerized local sandbox? Check out our [Docker Setup Guide](/docs/building-your-solution/deployment/self-hosting).
:::
...
- **Database (PostgreSQL or Azure SQL)**: Endatix supports both **Azure Database for PostgreSQL Flexible Server** and **Azure SQL Database (MS SQL Server)**. For PostgreSQL, allowlist `pg_trgm` (`azure.extensions` must include `PG_TRGM`) before migrations — see [System Requirements](/docs/getting-started/system-requirements#postgresql-pg_trgm-extension). The sample Bicep Postgres module sets this parameter for you.
...
- **Allowlist `pg_trgm` on PostgreSQL Flexible Server**: Required before API migrations when using PostgreSQL — see [System Requirements](/docs/getting-started/system-requirements#postgresql-pg_trgm-extension).
