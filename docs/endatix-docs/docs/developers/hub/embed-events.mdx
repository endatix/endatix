...
Host-page lifecycle for an Endatix embed: `endatix:form-loaded`, `endatix:form-complete`, and
`endatix:form-error`. Snippet attributes, height modes (`auto` / `fill`), and the local
cross-origin playground are in [Embed a Form via iframe](/docs/guides/embed-form-via-iframe).

Tokens and prefilling: [Form Prefilling and Sharing](/docs/guides/form-prefilling).
...
The script inserts an iframe immediately after the script tag. Each embed receives a unique `embedId`, so multiple forms can live on the same page without cross-handling events. Height modes (`auto` / `fill`) and the rest of the attributes: [Embed a Form via iframe](/docs/guides/embed-form-via-iframe).
...
Use the API **embed playground** (`GET /dev/embed-host`) so you see validated `CustomEvent`s on a
real cross-origin host — [Embed a Form via iframe](/docs/guides/embed-form-via-iframe#test-on-a-real-host).
The [basic embed sample](https://github.com/endatix/endatix/tree/main/samples/form-embedding/basic-embed)
is a static HTML fallback if WebHost is not running.

Do not open `/embed/{formId}` alone; that is the iframe document, not a host.
