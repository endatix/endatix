...
Next.js **inlines** any `NEXT_PUBLIC_*` variable into client JavaScript at build time. That cannot be overridden later from `process.env` — see [Next.js environment variables](https://nextjs.org/docs/app/guides/environment-variables#bundling-environment-variables-for-the-browser).
