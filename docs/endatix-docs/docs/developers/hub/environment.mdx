...
Next.js **inlines** any `NEXT_PUBLIC_*` environment variable into any JavaScript sent to the browser and this cannot be overrided even if they are set on the process.env as this is the [Next.js's design choice](https://nextjs.org/docs/app/guides/environment-variables#bundling-environment-variables-for-the-browser) on how bundling of env variables (inlining) for the browser should work. 

To better support containerized deployment scenarios like Docker or Kubernetes, the Endatix Hub’s browser-facing settings are deliberately **not** using `NEXT_PUBLIC_*` prefix - instead they use `ENDATIX_*` names resolved **at request time** on the server, then passed into the client tree. This allows you to set them up like standard Node.js environment variables
