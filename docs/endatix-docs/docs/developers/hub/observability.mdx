...
Noisy requests Hub serves — `/_next/*`, static files, RSC payload requests (`_rsc=`), `/api/health`, `robots.txt`, `sitemap` — are dropped for the **whole trace**, including the Next.js render spans under them, for Azure and OTLP alike. Internal `metric.*` spans and calls to `telemetry.nextjs.org` are dropped too. Outgoing calls are never dropped for their path: fetching an image or a `.json` file is a real dependency.

Two limits: static-file requests still export Next.js's two wrapper spans (they carry no URL to match), and with a rate limit those requests count against the per-second budget. Serving `/_next/static` from a CDN removes both.
