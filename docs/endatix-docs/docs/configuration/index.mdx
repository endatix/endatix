...
This guide covers **Endatix API** configuration (`appsettings.json` and `ConfigureEndatix()`). It applies whether you installed from NuGet or from source.

:::info[Looking for Hub `.env`?]
Endatix Hub is a Next.js app. Its keys live in [Hub environment variables](/docs/developers/hub/environment), not here.
:::
...
Side effects such as [webhook delivery](/docs/guides/webhooks) run through Endatix **background processing** and the **transactional outbox**. Configure the in-process relay under `Endatix:Outbox`—see [Background Processing](/docs/configuration/background-processing) for the pattern, defaults, and tuning options.
