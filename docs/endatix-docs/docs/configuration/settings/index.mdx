...
Endatix API exposes application settings, which allow you to configure the behavior of the applicaiton. They are part of the ASP.NET Core configuration framework and support per environment variations similar to .env files with note. More about ASP.NET Configuration [here](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/configuration/?view=aspnetcore-10.0#appsettingsjson).

For each project that you have setup via the [NuGet packages](/docs/getting-started/setup-nuget-package), you can apply the settings via the respective `appsettings.{Environment}.json` files there. By convention all settings will be part of the **Endatix** object in the root of your settings, so there's virtually no change of name colission with other settings you might have.. an it's also cleaner. Here is an example:
