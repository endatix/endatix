...
public sealed class UpdateStatusTests
{
    ...
    public async Task ExecuteAsync_WhenSuccessful_ShouldReturnOkResult()
    {
        ...
        var result = Result.Success(new SubmissionDto(submissionId, false, "{}", formId, 1, null, DateTime.UtcNow, null, DateTime.UtcNow, null, null, status, "7", 7, "7", null, false));
    ...
    public async Task ExecuteAsync_ShouldPassCorrectCommandToMediator()
    {
        ...
        var request = new UpdateStatusRequest(submissionId, formId, status);
        var result = Result.Success(new SubmissionDto(submissionId, false, "{}", formId, 1, null, DateTime.UtcNow, null, DateTime.UtcNow, null, null, status, null, null, null, null, false));
