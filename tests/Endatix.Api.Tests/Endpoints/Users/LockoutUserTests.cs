...
public class LockoutUserTests
{
    private readonly IMediator _mediator;
    private readonly LockoutUser _endpoint;

    public LockoutUserTests()
    {
        _mediator = Substitute.For<IMediator>();
        _endpoint = Factory.Create<LockoutUser>(_mediator);
    }
    ...
    public async Task ExecuteAsync_InvalidRequest_ReturnsProblemDetails()
    {
        ...
        _mediator.Send(Arg.Any<LockoutUserCommand>(), Arg.Any<CancellationToken>())
            .Returns(result);
    ...
    public async Task ExecuteAsync_ValidRequest_ReturnsOkWithUserOperation()
    {
        ...
        okResult.Value.Message.Should().Be("User locked out.");
    }
