...
public sealed class StorageUrlRewriteTransformerTests
{
    ...
    public void Transform_ReturnsValueUnchanged_WhenArrayWithNoStorageUrls()
    {
        ...
        var result = sut.Transform<SubmissionExportRow>(doc.RootElement, Ctx());
    ...
    public void Transform_RewritesStringifiedArray_WhenStorageUrlMatches()
    {
        ...
        using var doc = JsonDocument.Parse(json);
        var result = sut.Transform<SubmissionExportRow>(doc.RootElement.GetProperty("val"), Ctx(Row(1465276700186116096L, 1466449599089606656L)));
        Assert.NotNull(result);
    ...
    public void Transform_ReturnsValueUnchanged_WhenNumber()
    {
        ...
        var result = sut.Transform<SubmissionExportRow>(doc.RootElement, Ctx());
        Assert.NotNull(result);
        Assert.Equal(123, Assert.IsType<JsonElement>(result).GetInt64());
    }
