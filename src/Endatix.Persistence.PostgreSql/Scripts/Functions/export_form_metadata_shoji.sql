...
-- Description: Generates Crunch.io-compatible Shoji format metadata (codebook) for form exports.
--              Produces variable definitions with proper types (numeric, text, categorical, datetime, multiple_response),
--              categories for categorical variables, and subvariables for checkbox questions.
--              Includes system columns and sanitizes column aliases for compatibility.
-- Parameters: @target_form_id - The ID of the form to generate metadata for
-- Returns: JSONB containing complete Shoji metadata structure
-- Database: PostgreSQL
-- =============================================
...
WITH RECURSIVE
    ...
    -- *** Filter to a single definitive top-level checkbox element ***
	...
	calculated_column_specs AS (
	    SELECT
	        cv."FormId",
	        regexp_replace(cv.calc_name, '[^a-zA-Z0-9_]+', '_', 'g') AS column_name,
	        'calculated' AS source_type,
	        cv.calc_name AS original_name,
	        cv.calc_name AS original_title,
	        'expression' AS original_type,
	        NULL::text AS choice_text
    ...
    all_export_columns AS (
		...
		UNION ALL
		SELECT "FormId", column_name, source_type, original_name, original_title, original_type, choice_text
		FROM calculated_column_specs
    ),
    ...
    shoji_json_variables AS (
            ...
            jsonb_strip_nulls(jsonb_build_object(
                ...
                'categories', CASE
                    WHEN raw.original_type IN ('radiogroup', 'dropdown') AND raw.source_type = 'simple' THEN
                         (
                            -- Isolate the single correct element before expanding choices to prevent duplication.
        ...
        WHERE source_type IN ('system', 'simple', 'nested', 'calculated')
    ),
