...
- Hub build-time values: `endatix-hub/.env.deploy` (minimum keys). For full list see `endatix-hub/.env.example`
...
- `endatix-hub/.env.deploy` (minimal Hub build-time values)

Both are local-only files and should not be committed.

Entrypoint behavior:

- Keep running the same local command from this folder: `node ./generate-quickstart-secrets.mjs`
...
| Build-time Hub values         | Local env file only                     | `endatix-hub/.env.deploy` before `pnpm build:standalone`                                        |
